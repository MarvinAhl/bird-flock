...
pred_norm_vel = 20;  % Predators fly at fixed velocity


% Pos [2] and Vel [2]
% Randomly initialized
bird_states = rand(n_birds, 4);
bird_states(:, 1) = (bird_states(:, 1) .* 2 - 1) .* spawn_radius;
bird_states(:, 2) = (bird_states(:, 2) .* 2 - 1) .* spawn_radius;
bird_states(:, 3) = (bird_states(:, 3) .* 2 - 1) .* norm_vel;
bird_states(:, 4) = (bird_states(:, 4) .* 2 - 1) .* norm_vel;

% Predator
n_pred = 5;
predators = nan(n_pred, 4);
...
bird_plots = [];
for i = 1:n_birds
    bird_plots(i) = plot(bird_states(i, 1), bird_states(i, 2), 'k.', 'MarkerSize', 7);
    if (i == 1)
        hold on;
    end
end
...
pred_plots = [];
for i = 1:n_pred
    pred_plots(i) = plot(predators(i, 1), predators(i, 2), 'r.', 'MarkerSize', 15);
end
...
flock_video = VideoWriter('flock.mp4', 'MPEG-4');
...
while true
    ...
    if (any(isnan(predators)))
        if (rand() < predator_prob)
            ...
            pred_ang = 2*pi * rand();
            pred_pos = pred_spawn_radius * [cos(pred_ang), sin(pred_ang)];
            pred_vel = - pred_norm_vel * [cos(pred_ang), sin(pred_ang)];
    ...
    new_bird_states = zeros(n_birds, 4);
    for i = 1:n_birds
        bird_pos = reshape(bird_states(i, 1:2), 2, 1);
        bird_vel = reshape(bird_states(i, 3:4), 2, 1);
        bird_pos_abs = norm(bird_pos);
        bird_vel_abs = norm(bird_vel);

        % Find closest bird neighbour
        bird_dists = vecnorm(bird_states(:, 1:2) - bird_pos', 2, 2);
        ...
        min_dist_pos = reshape(bird_states(min_dist_bird_idx, 1:2), 2, 1);
        ...
        for j = 1:n_birds
            ...
            other_bird_pos = reshape(bird_states(j, 1:2), 2, 1);
            other_bird_vel = reshape(bird_states(j, 3:4), 2, 1);
        ...
        for j = 1:n_pred
            ...
            pred_pos = reshape(predators(j, 1:2), 2, 1);
        ...
        new_bird_states(i, :) = [reshape(new_bird_pos, 1, 2), reshape(new_bird_vel, 1, 2)];
    end
    ...
    for i = 1:n_pred
        if (any(isnan(predators(i, :))) || norm(predators(i, 1:2)) > pred_spawn_radius+1)
            predators(i, :) = nan(1, 4);
            continue;
        end

        predators(i, 1:2) = predators(i, 1:2) + predators(i, 3:4) * dt;
    end

    % Update animation
    for i = 1:n_birds
        bird_pos = bird_states(i, 1:2);
        set(bird_plots(i), 'XData', bird_pos(1), 'YData', bird_pos(2));
    end
    % Update predator animation
    for i = 1:n_pred
        pred_pos = predators(i, 1:2);
        set(pred_plots(i), 'XData', pred_pos(1), 'YData', pred_pos(2));
    end
    drawnow;
